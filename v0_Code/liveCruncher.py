...
def getCalcAllPastTrades(market, id_col, trades, endSeconds):
    ...
    return trades, data[2]['avgPrice'], data[5]['avgPrice'], data[8]['avgPrice'], data[10]['avgPrice'], data[11]['avgPrice']
...
def getLatestNumbers(trades):
    ...
    for row in rows:
        ...
        trades, tSecAvg, fiveAvg, thirtyAvg, oneTwentyAvg, fourEightyAvg = getCalcAllPastTrades(market, row[0], oldTrades, round(row[1] / 1000))

    return trades, tSecAvg, fiveAvg, thirtyAvg, oneTwentyAvg, fourEightyAvg
