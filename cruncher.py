...
def getCalcAllTrades(market, id_col, end_ms):
    ...
    for period in data:
        ...
        # cur.execute(
        #     "UPDATE trades SET {0:s}avgPrice = {1:0.8f}, {2:s}highPrice = {3:0.8f}, {4:s}lowPrice = {5:0.8f}, {6:s}startPrice = {7:0.8f}, {8:s}endPrice = {9:0.8f}, {10:s}changeReal = {11:0.8f}, {12:s}changePercent = {13:0.8f}, {14:s}travelReal = {15:0.8f}, {16:s}travelPercent = {17:0.8f}, {18:s}volumePrMin = {19:0.8f}, {20:s}tradesPrMin = {21:0.8f}, {22:s}buysPrMin = {23:0.8f}, {24:s}sellsPrMin = {25:0.8f} WHERE id = {26:d}".format(prepend, period['avgPrice'], prepend, period['highPrice'], prepend, period['lowPrice'], prepend, period['startPrice'], prepend, period['endPrice'], prepend, period['changeReal'], prepend, period['changePercent'], prepend, period['travelReal'], prepend, period['travelPercent'], prepend, period['volumePrMin'], prepend, period['tradesPrMin'], prepend, period['buysPrMin'], prepend, period['sellsPrMin'], id_col))
        # conn.commit()
...
while True:
    cur.execute("SELECT id, date_ms FROM trades WHERE p_oneTwentyMin_changePercent IS NULL ORDER BY date_ms DESC LIMIT 10")
    rows = cur.fetchall()
