...
class marketSourceCoinEx:
    ...
    def isLongTermTrendVeryUp(self):
        if self.longTermTrendPrediction() > ( 2 * self.feeAsPercent ):
            return True
        return False

    def isLongTermTrendVeryDown(self):
        if self.longTermTrendPrediction() < 0 - ( 2 * self.feeAsPercent ):
            return True
        return False
        ...
        try:
            ...
            for i in range(0, self.r.llen('expiringTrades')):
                try:
                    expiringTrades = True
                    trade = pickle.loads(self.r.lindex('expiringTrades', i))
                    print("Price: {0:0.8f} type: {1:4s} expires: {2:s}".format(trade['price'], trade['tradeType'], trade['expires'].astimezone(timezone('US/Central')).strftime("%Y-%m-%d %I:%M:%S%p")))
                except TypeError:
                    print("Pickle Problems")
            ...
            if self.priceChange < 0.0:
                print("     price down: %0.8f" %abs(self.priceChange))
                print("     price down: %0.2f percent" %abs(self.percentChange))
            elif self.priceChange > 0.0:
                print("       price up: %0.8f" %abs(self.priceChange))
                print("       price up: %0.2f percent" %abs(self.percentChange))
            else:
